...
export default () => {
  const [data, values, setValues] = Calculator.useData(DEFAULT_DATA, (settings) => ({
    numSpins: settings.numSpins ? parseInt(settings.numSpins, 10) : undefined,
    numDays: settings.numDays ? parseInt(settings.numDays, 10) : undefined,
  ...
  if (data.numSpins % data.numDays !== 0) {
    let spinsPerDay = Math.floor(data.numSpins / data.numDays);
    let extraSpinDays = data.numSpins % data.numDays;
    let regularDays = data.numDays - extraSpinDays;
    ...
    let coinsPerDay = 0;
    let coinsPerExtraSpinDay = 0;
    for (let i = 0; i < (spinsPerDay + 1); i++)
    {
        coinsPerExtraSpinDay += i*wheelCost;
    }
    ...
    let spinsPerDay = data.numSpins / data.numDays;
  ...
  return <Calculator pageTitle="Wheel Calculator" valueSetter={setValues}>
    <Input.Text
        id="numSpins"
